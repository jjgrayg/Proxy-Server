...
void ProxyServer::close_connection(con_handle_t con_handle) {
    ...
    try {
        if (debugging_)
            write_to_outputs("<span style=\"color:blue\">DEBUGGING:: </span> Closing Connection! to " + con_handle->response_socket_.lowest_layer().remote_endpoint().address().to_string() + "\n");
        con_handle->response_socket_.shutdown();
    }
    catch(boost::system::system_error e) {}
    ...
    if (!e) {
        ...
        if (debugging_) {
            write_to_outputs("<span style=\"color:blue\">DEBUGGING:: </span>Incoming connection from: " + con_handle->request_socket_.remote_endpoint().address().to_string() +
                             " and to: " + con_handle->response_socket_.lowest_layer().remote_endpoint().address().to_string() + ". Requested path: " + con_handle->remote_host_ + "\n");
        }
        if (req_type != "CONNECT") {
...
void ProxyServer::establish_up_connection(con_handle_t con_handle, string const& host, string const& req_type) {
    ...
    if (sv.size() > 0) {
        ...
        try {
            con_handle->response_socket_.set_verify_mode(boost::asio::ssl::verify_peer);
            con_handle->response_socket_.set_verify_callback(
                boost::bind(&ProxyServer::verify_certificate, this, _1, _2));
            string host = sv[0];
            ...
            auto connect_handler = boost::bind(&ProxyServer::handle_up_connect, this, con_handle, req_type, host, boost::asio::placeholders::error);
...
//            con_handle->response_socket_.async_connect(iter->endpoint(), connect_handler);
            boost::asio::async_connect(con_handle->response_socket_.lowest_layer(), iter, connect_handler);
        }
...
void ProxyServer::handle_up_connect(con_handle_t con_handle, string const& req_type, string const& host, boost::system::error_code const& e)
{
  if (!e)
  {
    auto handshake_handler = boost::bind(&ProxyServer::handle_bidirectional_connection, this, con_handle, req_type, host, boost::asio::placeholders::error);
    con_handle->response_socket_.async_handshake(boost::asio::ssl::stream_base::client, handshake_handler);
  }
  else
  {
    write_to_outputs("<span style=\"color:red\">!!ERROR!! </span>" + e.message() + " at line #" + std::to_string(__LINE__) + "\n");
...
// Verifies the certificate present (NEEDS ACTUAL IMPLEMENTATION)
bool ProxyServer::verify_certificate(bool preverified, boost::asio::ssl::verify_context& ctx) {
    ...
    write_to_outputs("<span style=\"color:green\">SERVER</span>:: Verifying \"" + std::to_string(port_) + "\"\n");
...
void ProxyServer::start_accept() {
    auto con_handle = m_connections_.emplace(m_connections_.begin(), m_ioservice_, ctx);
    ...
    try {
        ...
        if (debugging_)
            write_to_outputs("<span style=\"color:blue\">DEBUGGING:: </span> Recieved " + std::to_string(con_handle->response_.size()) +
                             ...
                             " seconds from " + con_handle->response_socket_.lowest_layer().remote_endpoint().address().to_string() + "\n");
...
void ProxyServer::e2e_connection_handler(con_handle_t con_handle, boost::system::error_code const& e) {
    if (!e && con_handle->request_socket_.is_open() && con_handle->response_socket_.lowest_layer().is_open()) {
...
void ProxyServer::write_connect_request(con_handle_t con_handle, boost::system::error_code const& e, size_t bytes_transferred) {
    if (!e && bytes_transferred != 0) {
        ...
        boost::asio::async_write(con_handle->response_socket_, boost::asio::buffer(con_handle->request_data_, bytes_transferred), handler);
             ...
             && bytes_transferred != 0 && con_handle->response_socket_.lowest_layer().is_open()) {
        ...
        if (e != boost::asio::error::eof && e != boost::asio::error::connection_reset)
            write_to_outputs("<span style=\"color:red\">!!ERROR!! </span>" + e.message() + " at line #" + std::to_string(__LINE__) + "\n");
        if (con_handle->response_socket_.lowest_layer().is_open()) {
            auto handler = boost::bind(&ProxyServer::write_and_close_request, this, con_handle, boost::asio::placeholders::error);
            boost::asio::async_write(con_handle->response_socket_, boost::asio::buffer(con_handle->request_data_, bytes_transferred), handler);
        }
    }
...
void ProxyServer::read_connect_response(con_handle_t con_handle, boost::system::error_code const& e) {
    if (!e && con_handle->response_socket_.lowest_layer().is_open()) {
...
void ProxyServer::write_and_close_request(con_handle_t con_handle, boost::system::error_code const& e) {
    ...
    try {
        con_handle->request_socket_.shutdown(boost::asio::ip::tcp::socket::shutdown_both);
        con_handle->request_socket_.close();
        if (!con_handle->request_socket_.is_open() && !con_handle->response_socket_.lowest_layer().is_open()) {
            write_to_log(con_handle->connect_time_ + ", " + get_current_date_and_time() + ", false, " + con_handle->remote_host_ + "\n");
            m_connections_.erase(con_handle);
        }
    }
