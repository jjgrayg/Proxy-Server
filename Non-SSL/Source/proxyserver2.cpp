...
void ProxyServer::write_to_outputs(string str) {
    writer_mutex_.lock();
    emit ready_write_console(QString::fromStdString("<span><span style=\"color:grey\">[" + get_current_date_and_time() + "]</span> ~ <span style=\"color:green\">SERVER</span>:: " + str + "\n"));
    if (logging_) {
        ...
        if (!log_writer_) emit ready_write_console(QString::fromStdString("span><span style=\"color:grey\">[" + get_current_date_and_time() +
                                                                          ...
                                                                          "<span style=\"color:red\">!!ERROR!! </span>Could not create log. Line #" +
...
string ProxyServer::request_to_str_buff(con_handle_t con_handle, size_t buff_size) {
    ...
    size_t size = get_expected_request_size(str);
    try {
        con_handle->request_ = str;
    }
    catch (std::logic_error const& e) {
        write_to_outputs("<span style=\"color:red\">!!ERROR!! </span>" + string(e.what()) + " line #" + std::to_string(__LINE__) + "\n");
        con_handle->request_ = "";
    }
...
// Reads buffer into string
string ProxyServer::response_to_str_buff(con_handle_t con_handle, bool header, size_t buff_size) {
    string str;
...
size_t ProxyServer::get_response_size(string const& response) {
    vector<string> vec = split_string(response.substr(0, response.find("\r\n\r\n")), ' ', '\r');
    auto it = std::find(vec.begin(), vec.end(), "\nContent-Length:");
    if (it != vec.end())
        return std::stoul(*(++it));
    return std::stoul("0");
...
size_t ProxyServer::get_expected_response_size(string const& response) {
    string str = response;
    size_t result = str.substr(0, str.find("\r\n\r\n")).size() + get_response_size(str) + 8;
    return result;
}

int ProxyServer::get_response_encoding(string const& response) {
    vector<string> vec = split_string(response.substr(0, response.find("\r\n\r\n")), ' ', '\r');
    auto it = std::find(vec.begin(), vec.end(), "\nTransfer-Encoding:");
    if (it != vec.end()) {
        if (*(++it) == "chunked")
...
size_t ProxyServer::get_request_size(string const& request) {
...
size_t ProxyServer::get_expected_request_size(string const& request) {
    string str = request;
    size_t result = str.substr(0, str.find("\r\n\r\n")).size() + get_request_size(str) + 8;
    return result;
}

int ProxyServer::get_request_encoding(string const& request) {
    vector<string> vec = split_string(request.substr(0, request.find("\r\n\r\n")), ' ', '\r');
    auto it = std::find(vec.begin(), vec.end(), "\nTransfer-Encoding:");
    if (it != vec.end()) {
        if (*(++it) == "chunked")
...
void ProxyServer::handle_local_request(con_handle_t con_handle) {
    auto c = con_handle->request_.substr(con_handle->request_.find(' ') + 1, con_handle->request_.find("/r/n"));
...
size_t ProxyServer::get_type(string const& buff) {
    vector<string> vec = split_string(buff.substr(0, buff.find("\r\n\r\n")), ' ', '\r');
    auto it = std::find(vec.begin(), vec.end(), "\nContent-Type:");
    if (it != vec.end()) {
        if ((*(++it)).find("image") != string::npos) {
            if ((*(++it)).find("jpeg") != string::npos) return 1;
            else if ((*(++it)).find("png") != string::npos) return 2;
            else if ((*(++it)).find("gif") != string::npos) return 3;
        }
        else if ((*(++it)).find("application") != string::npos) return 4;
    }
    return 0;
}

string convert_to_type(size_t type) {
    switch(type) {
    case 0:
        return "text/html";
    case 1:
...
void ProxyServer::handle_parse(con_handle_t con_handle, boost::system::error_code const& e, size_t bytes_transfered) {
    ...
    if ((!e || e == boost::asio::error::eof) && bytes_transfered != 0) {
        ...
        if (debugging_) {
            write_to_outputs("<span style=\"color:blue\">DEBUGGING:: </span>New connection from: " + con_handle->request_socket_.remote_endpoint().address().to_string() + "\n");
        }
        establish_up_connection(con_handle, host, req_type);
    }
    else
        write_to_outputs("<span style=\"color:red\">!!ERROR!! </span>" + e.message() + " line #" + std::to_string(__LINE__) + "\n");
}
    ...
    if (!e) {
        if (debugging_) {
            write_to_outputs("<span style=\"color:blue\">DEBUGGING:: </span>Incoming connection from: " + con_handle->request_socket_.remote_endpoint().address().to_string() +
                             " and to: " + con_handle->response_socket_.remote_endpoint().address().to_string() + "\n");
        }
...
void ProxyServer::establish_up_connection(con_handle_t con_handle, string const& host, string const& req_type) {
    ...
    string port;
    if (sv.size() < 2) port = "http";
    else port = sv[1] == "443" ? "443" : "http";
...
void ProxyServer::do_http_routine(con_handle_t con_handle, string const& host) {
    ...
    if (result != 1) { // Cache miss
        if (debugging_) write_to_outputs("<span style=\"color:blue\">DEBUGGING:: </span> <span style=\"color:red\">Cache miss</span>\n");
        ...
        emit cache_hit(++cache_hits);
        if (debugging_) write_to_outputs("<span style=\"color:blue\">DEBUGGING:: </span> <span style=\"color:green\">Cache hit</span>\n");
        size_t type = cache_manager_->get_file_type();

        vector<unsigned char> proxy_res = cache_manager_->consume_binary();

        char char_buff[proxy_res.size()];
        std::copy(proxy_res.begin(), proxy_res.end(), char_buff);
        emit add_cache_bytes(cache_hit_size += proxy_res.size());

        string response = "HTTP/1.1 200 OK\r\nContent-Length: " + std::to_string(proxy_res.size()) + "\r\n\r\n" + char_buff;
        unsigned char buff[response.size()];
        for(size_t i = 0; i < response.size(); ++i) buff[i] = response[i];

        auto handler = boost::bind(&ProxyServer::close_connection, this, con_handle, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred, time_since_epoch());
        boost::asio::async_write(con_handle->request_socket_, boost::asio::buffer(buff, response.size()), handler);
...
void ProxyServer::read_http_response(con_handle_t con_handle, string const& cache_filename, boost::system::error_code const& e, size_t bytes_transferred) {
    if (!e) {
        ...
        temp += response_to_str_buff(con_handle, true, bytes_transferred);
        con_handle->response_ = temp;
        size_t t = temp.size();
        int encode_t = get_response_encoding(con_handle->response_);
        ...
        else if (encode_t == 1 && t_vec[1] == "200" && bytes_transferred != 0) {
            string delim = "\r\n0\r\n";
        ...
        else if (encode_t == 0 && t_vec[1] == "200") {
            size_t buff_size = get_response_size(con_handle->response_);
...
void ProxyServer::close_connection(con_handle_t con_handle, boost::system::error_code const& e, size_t bytes_transferred, double start_time) {
    if (debugging_)
...
void ProxyServer::write_http_response(con_handle_t con_handle, string const& cache_filename, double time_read, size_t buff_size) {
    try {
        string temp = con_handle->response_;
        temp += response_to_str_buff(con_handle, false, buff_size);
        con_handle->response_ = temp;
        vector<string> t_vec = split_string(con_handle->response_.substr(0, con_handle->response_.find("\r\n")), ' ');
        double time = time_since_epoch();
        unsigned char buff[temp.size()];
        for (size_t i = 0; i < temp.size(); ++ i) buff[i] = temp[i];
        if (debugging_)
        ...
        auto handler = boost::bind(&ProxyServer::close_connection, this, con_handle, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred, time);
        boost::asio::async_write(con_handle->request_socket_, boost::asio::buffer(buff, temp.size()), handler);
        if (t_vec.size() > 1 && t_vec[1] == "200") {
            string body = temp.substr(temp.find("\r\n\r\n") + 4, temp.size());
            vector<unsigned char> binary_file(body.size());
            for (size_t i = 0; i < body.size(); ++i) {
                binary_file[i] = body[i];
            }
            size_t type = get_type(temp);
            cache_manager_->add_file_to_cache(cache_filename, binary_file, type, get_max_age(temp), get_age(temp));
        }
    }
...
void ProxyServer::write_connect_request(con_handle_t con_handle, boost::system::error_code const& e, size_t bytes_transferred) {
    ...
    else if (e == boost::asio::error::eof && bytes_transferred != 0) {
